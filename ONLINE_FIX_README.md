# Online Play Fix — Firebase Realtime Database
...
- ✅ Opponent disconnect detection via `onDisconnect` + presence markers.
- ✅ "Claim Win" button when the opponent has disconnected.
- ✅ Per-tab identity (sessionStorage) so you can test in two tabs.
- ✅ The public hook API is backward-compatible — `OnlineSocketProvider`, `OnlineLobbyView`, and `OnlineGameView` needed only minor additive changes.

## Files changed

| File | Change |
|------|--------|
| `src/lib/firebase.ts` | **NEW** — Firebase initialization using your provided config. Supports `NEXT_PUBLIC_FIREBASE_DATABASE_URL` env override. |
| `src/hooks/useOnlineChess.ts` | **REWRITTEN** — socket.io replaced with Firebase Realtime DB. Same public API plus `opponentConnected` and `claimWin`. |
| `src/components/chess/OnlineGameView.tsx` | Added opponent-disconnect banner + "Claim Win" button. |
| `src/components/chess/OnlineLobbyView.tsx` | Auto-join now waits for `connected` instead of a 500ms timeout. |
| `package.json` | Added `firebase: ^10.14.1` to dependencies. |

The old socket.io mini-service (`mini-services/chess-online/`) and the `Caddyfile` are now **unused** — you can delete them or leave them; they don't affect anything.
...
This is already set as the default in `src/lib/firebase.ts`. If your database is in a different region (e.g. Europe), the URL will be different — either edit it in the file or set the `NEXT_PUBLIC_FIREBASE_DATABASE_URL` environment variable (recommended for production).
...
Deploy to your host as usual (Vercel, Netlify, etc.). No environment variables are needed for online play unless you used `NEXT_PUBLIC_FIREBASE_DATABASE_URL` in step 2.
...
5. Close tab A. Tab B should show an amber "Opponent disconnected" banner with a **Claim Win** button after a moment.
...
### Player identity (per-tab)

Each browser tab gets a stable random UUID stored in **`sessionStorage`** (`grandmasters-arena-online-client-id`). This:

- Lets you test online play by opening the app in two tabs (each tab gets a different id).
- Preserves your seat across refreshes within the same tab.

The original `localStorage` approach broke two-tab testing because both tabs shared one id — the second tab couldn't join a room the first tab had created (the join transaction rejected it as "already host").

### Opponent disconnect detection

When a player creates or joins a room, the hook writes a presence marker under `chess/rooms/{CODE}/presence/{clientId}` and queues an `onDisconnect` removal for it. Firebase executes the queued removal server-side when the client's connection drops — even if the client goes away ungracefully (tab close, network loss, etc.).

The other player's `onValue` listener derives `opponentConnected` from whether the opponent's presence marker exists. When it goes false, `OnlineGameView` shows an amber banner with a **Claim Win** button.

**Claim Win** is transactional: it checks that the opponent's presence is actually gone before marking the game as abandoned. If the opponent reconnects (e.g., they were just refreshing), the claim is rejected.
...
Before the transaction, we pre-generate a unique `moveKey` (e.g., `m1700000000000_abc123`). Inside the transaction, we use this key when adding the move. After the transaction commits, we check if `snapshot.val().moves[moveKey]` exists. If not, the move was rejected (illegal under latest FEN, or it wasn't the sender's turn, or the room vanished) — we surface an `error` so `OnlineGameView` can revert its optimistic UI.

### Chat pagination

The chat listener uses `query(chatRef, limitToLast(50))` for the initial load, so re-mounting on a long-running room doesn't replay hundreds of old messages. New messages arriving after mount are delivered via `onChildAdded` with deduplication.

### Cleanup on unmount

When `OnlineSocketProvider` unmounts (user leaves the online flow entirely), the hook:

- Tears down all Firebase listeners.
- If the user was in a **waiting** room as host: deletes the room and its lobby entry (no one else is there, so it would just leak).
- If the user was in a **playing** or **finished** room: removes their presence marker (so the opponent can detect the disconnect and claim the win). Does NOT mark the game as abandoned — the user might be refreshing, and the opponent has the `claimWin` path if they're really gone.
...
### Option B: Rate-limit writes

You can add Firebase RTDB rules to limit how fast a single client can write (e.g. throttle chat). See the Firebase docs on `data.exists()` and timestamp validation.
...
You'll see a "Permission denied" or "Database does not exist" error in the browser console. Follow step 1 above to create the database. Make sure you create a **Realtime Database**, not a Firestore Database.
...
**Q: What happens if I close my tab mid-game?**

Firebase's `onDisconnect` queue fires, removing your presence marker. Your opponent sees an "Opponent disconnected" banner and can click **Claim Win** to end the game. If you re-open the app before they claim, you can create/join a new room (your old room will eventually be cleaned up or claimed).

**Q: What happens if I refresh mid-game?**

Your presence marker is momentarily removed (during the refresh), then re-established when the new page load calls `setupPresence`. Your opponent might briefly see "Opponent disconnected" but it'll recover within seconds. The room state is preserved in Firebase, so you'll re-subscribe to it on the new page load — but note that the current implementation doesn't auto-rejoin rooms on refresh (you'd need to manually re-enter the room code or use the share link).

**Q: Can I test online play by opening two tabs?**

Yes. Each tab gets its own client id (stored in `sessionStorage`, which is per-tab). Create a room in tab A, copy the code, join from tab B. Both tabs will see each other as separate players.
