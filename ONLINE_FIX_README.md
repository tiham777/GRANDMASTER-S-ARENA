...
After the transaction commits, we read the latest move's `by` field. If it doesn't match the sender's color, the move was rejected (illegal under latest FEN, or it wasn't the sender's turn). We surface an `error` so `OnlineGameView` can revert its optimistic UI.
