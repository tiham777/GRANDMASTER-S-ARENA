...
export function useOnlineChess() {
  // ---- Connection + identity ----------------------------------------------
  // The per-tab client id is initialized lazily via useState's initializer
  // (NOT in an effect) so we avoid a synchronous setState-in-effect
  // (react-hooks/set-state-in-effect). This is safe because
  // OnlineSocketProvider is only mounted client-side (view starts at "home"
  // on the server), so getClientId() always sees a browser environment.
  const clientIdRef = useRef<string>("");
  const [connected, setConnected] = useState(false);
  const [socketId, setSocketId] = useState<string | null>(() => {
    const id = getClientId();
    return id !== "ssr-noop" ? id : null;
  });
  ...
  // Rematch state — declared up here (not later) so every effect / callback
  // below can safely reference the setters without a forward-declaration
  // lint error (react-hooks/immutability) or TDZ risk.
  ...
  // ---- Boot: monitor connection health ------------------------------------
  // (Client id is initialized lazily via useState above; here we just mirror
  // it into clientIdRef for use in callbacks, and set up the Firebase
  // `.info/connected` listener + presence re-establishment on reconnect.)
  useEffect(() => {
    const clientId = socketId ?? getClientId();
  ...
  // ============================================================
  //
  // NOTE: `myColor` MUST be declared before any effect that references it
  // in a dependency array. Earlier this was declared at the very end of the
  // hook, which caused a Temporal Dead Zone ReferenceError on every render
  // (the beforeunload effect's deps array below evaluated `myColor` before
  // the `const` was initialized). Moving it up here fixes the crash.
