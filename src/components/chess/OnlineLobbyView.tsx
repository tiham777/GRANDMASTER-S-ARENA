...
export function OnlineLobbyView({ onBack, onGameStart, initialJoinCode }: OnlineLobbyViewProps) {
  ...
  // Waits for the Firebase connection to be live before joining (instead
  // of a fixed 500ms delay that can race on slow networks).
  const didAutoJoinRef = useRef(false);
  useEffect(() => {
    if (didAutoJoinRef.current) return;
    if (!initialJoinCode || online.room) return;
    if (!online.connected) return; // wait until Firebase is online
    const saved = typeof window !== "undefined" ? localStorage.getItem(NAME_KEY) : null;
    if (!saved) return;
    didAutoJoinRef.current = true;
    online.joinRoom(initialJoinCode, saved);
  }, [initialJoinCode, online.connected, online.room]);
