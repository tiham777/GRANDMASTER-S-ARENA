...
export function OnlineGameView({ onExit, onGameEnded }: OnlineGameViewProps) {
  ...
  return (
    <div className={`game-container mx-auto w-full max-w-7xl px-2 py-1 sm:px-4 sm:py-1.5 ${focusMode ? "focus-mode" : ""} ${matchOpening ? "match-opening" : ""}`}>
      ...
      {/* Opponent disconnect banner + Claim Win */}
      {room.status === "playing" && !online.opponentConnected && online.connected && (
        <motion.div
          initial={{ opacity: 0, y: -8 }}
          animate={{ opacity: 1, y: 0 }}
          className="mb-2 flex flex-col items-center justify-between gap-2 rounded-lg border border-amber-500/40 bg-amber-500/10 px-4 py-2 text-xs sm:flex-row"
        >
          <span className="flex items-center gap-2 text-amber-300">
            <WifiOff className="h-3.5 w-3.5" />
            Opponent disconnected. If they don&apos;t return, you can claim the win.
          </span>
          <Button
            size="sm"
            variant="outline"
            className="h-7 gap-1 border-amber-500/50 text-amber-300 hover:bg-amber-500/20"
            onClick={() => online.claimWin(room.code)}
          >
            <Trophy className="h-3.5 w-3.5" /> Claim Win
          </Button>
        </motion.div>
      )}
